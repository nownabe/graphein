...
description: >-
  Run a single-pass code review using Claude.
  Reports structured findings to the user for interactive triage.
disable-model-invocation: false
allowed-tools:
  - Bash(bun run tools/review-by-claude.ts *)
  - Read
...
You are running a code review using Claude via `tools/review-by-claude.ts`.
Arguments: $ARGUMENTS

## Run Review

```
bun run tools/review-by-claude.ts
```
